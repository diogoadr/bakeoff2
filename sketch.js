...
function createTargets(target_size, horizontal_gap, vertical_gap, windowWidth, windowHeight) {
  ...
  for (key in indexed_array) {
    ...
    if (indexed_array[key].length > 1) {
      ...
      let menu_y = windowHeight / 2 + (radius * menus + 55 / menus) * Math.sin(angle - Math.PI / 2);

      let targetMenu = new Target(menu_x, menu_y, target_size, key, MENU_ID, indexed_array[key]);
